...
JNIEXPORT void JNICALL Java_com_hanhuy_android_keepshare_NdkAESEngine_scheduleKey
  ...
  WORD out[60];
  BYTE keydata[32];
  int len = (*env)->GetArrayLength(env, key);
  (*env)->GetByteArrayRegion(env, key, 0, 32, keydata);
  aes_key_setup(keydata, out, len * 8);
  (*env)->SetIntArrayRegion(env, scheduleOut, 0, 60, out);
}

JNIEXPORT void JNICALL Java_com_hanhuy_android_keepshare_NdkAESEngine_encrypt
  (JNIEnv *env, jclass cls, jbyteArray in, jint inoff, jbyteArray out, jint outoff, jintArray keysched, jint keysize) {
  BYTE block[16];
  WORD schedule[60];
  (*env)->GetByteArrayRegion(env, in, inoff, 16, block);
  (*env)->GetIntArrayRegion(env, keysched, 0, 60, schedule);
  aes_encrypt(block, block, schedule, keysize);
  (*env)->SetByteArrayRegion(env, out, outoff, 16, block);
}

JNIEXPORT void JNICALL Java_com_hanhuy_android_keepshare_NdkAESEngine_decrypt
  (JNIEnv *env, jclass cls, jbyteArray in, jint inoff, jbyteArray out, jint outoff, jintArray keysched, jint keysize) {
